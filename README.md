...
基于 **Go** + **Gin** + **gorilla/websocket** 构建的高性能实时弹幕系统，支持 B站 风格的弹幕类型（滚动/顶部/底部/逆向）、**用户认证（JWT）**、**直播间状态管理**、房间隔离、多存储后端、Redis 跨实例广播、异步持久化、Prometheus/Grafana 可观测性与优雅关闭。
...
> 未配置 `auth.jwt_secret` 时系统自动生成随机密钥，实例重启后所有已有 token 失效。
> 生产环境务必在 `config.yaml` 或环境变量中固定 `AUTH_JWT_SECRET`。
...
支持的环境变量：`SERVER_PORT`、`STORE_DSN`、`REDIS_ADDR`、`REDIS_INSTANCE_ID_PREFIX`、`LOG_LEVEL`、`LOG_FORMAT`、`AUTH_JWT_SECRET`。
...
│   ├── danmaku.go              # 弹幕业务逻辑（频率限制、校验、异步写入）
...
| `AUTH_JWT_SECRET` | `auth.jwt_secret` | JWT 签名密钥 |
